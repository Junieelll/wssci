    ...
    <div id="product-detail-container" class="bg-white overflow-hidden">
      ...
      <div class="max-w-7xl mx-auto px-6 lg:px-16 py-12 lg:py-20
            ...
            <div class="reveal relative group">
              ...
              <div id="pd-thumbnails" class="mt-4 flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                ${(p.images && p.images.length > 1) ? p.images.map((imgUrl, i) => `
                  ...
                  >
                    <img src="${imgUrl}" alt="${p.name} thumbnail" class="w-full h-full object-contain">
                  </button>
                `).join('') : ''}
              </div>
            </div>
            ...
            <div class="reveal-right delay-75">
              <div class="bg-surface-2 border border-brand-muted/15 rounded-3xl p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 shadow-sm">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto shrink-0">
                  <a href="#/contact"
                     class="flex items-center justify-center gap-2 px-6 py-3 rounded-full font-display font-700 text-[13px] text-brand bg-brand/10 hover:bg-brand/20 active:scale-95 transition-all w-full sm:w-auto text-center shrink-0 border border-brand/20">
                  ...
                  <a href="https://www.facebook.com/messages/t/61554434231420/" target="_blank"
                     class="flex items-center justify-center gap-2 px-6 py-3 rounded-full font-display font-700 text-[13px] text-white bg-[#00B2FF] hover:bg-[#009EE6] active:scale-95 transition-all shadow-md hover:shadow-lg w-full sm:w-auto text-center shrink-0">
                ...
                <div class="flex items-center gap-2.5 flex-wrap" id="pd-color-swatches">
                  ${colors.map((c, i) => `
                    <button
                      ...
                      data-color-images="${c.images ? c.images.join(',') : ''}"
                      data-color-sizes="${c.sizes ? c.sizes.join('|') : ''}"
                      title="${c.name}"
        ...
        : ''}
            ...
            <!-- ── PRODUCT INFO SECTIONS: Benefits / Specs / Certifications ── -->
          ...
          if (images.length > 1) {
            thumbnailsCont.innerHTML = images.map((imgUrl, i) => `
              <button
                ...
                class="pd-thumb-btn relative w-16 h-16 rounded-xl overflow-hidden border-2 ${i === 0 ? 'border-brand' : 'border-transparent'} hover:border-brand/50 transition-colors shrink-0 bg-white shadow-sm flex items-center justify-center p-2"
              >
                <img src="${imgUrl}" alt="${name} thumbnail" class="w-full h-full object-contain">
              </button>
            `).join('');
          } else {
            thumbnailsCont.innerHTML = ''; // Hide thumbnails if only 1 image
          }
        }
    ...
    if (infoSections) {
      infoSections.addEventListener('click', (e) => {
        ...
        if (isExpanded) {
          ...
          // After transition, remove max-height so content can flex naturally
          const onEnd = () => {
